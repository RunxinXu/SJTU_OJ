...
//对于动态规划 那么可以从后往前找 初始点是那个f（） 值最大的点 之后往后扫 找到 f()比它少1 而且比他小的 就是下一个
// 由于是顺序的 所以线性时间~


//对于nlgn做法的 b中储存的并不是！！
//比如 1 5 8 3 那么b中最后储存的是 138 并非所要的
//究其原因 就是在前面的被改动了但没有发挥继续延长的作用 ~
//对于这种nlgn的方法 没办法输出LIS！


//LIS 还可以用 LCS 做。。
//我们先对序列排序 然后找排序后的和原来的 他们的LCS。。。。
//6666！